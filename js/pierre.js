document.addEventListener('mousemove', function(event) {
    ...
    pierres.forEach(function(pierre) {
      ...
      const moveFactor = Math.min(distance / 20, 120);  // L'effet sera plus marqué, jusqu'à un maximum de 100
      
      // Calculer les décalages, amplifiés par la distance
      const deltaX = distanceX / (10 + moveFactor); // Moins de division pour plus d'amplitude
      const deltaY = distanceY / (10 + moveFactor); // Moins de division pour plus d'amplitude
      
      // Appliquer la transformation de mouvement
